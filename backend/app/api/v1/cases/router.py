...
async def create_case(
    ...
    case_service = CaseService(db)
    case = await case_service.create_case(payload, actor_id=token_data.sub or "usr-alex-01")
...
async def update_case(
    ...
    case_service = CaseService(db)
    case = await case_service.update_case(case_id, payload, actor_id=token_data.sub or "usr-alex-01")
    return ResponseEnvelope(success=True, data=case)
...
async def delete_case(
    ...
    case_service = CaseService(db)
    await case_service.soft_delete_case(case_id, actor_id=token_data.sub or "usr-alex-01")
