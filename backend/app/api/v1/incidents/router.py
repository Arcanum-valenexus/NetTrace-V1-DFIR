...
async def list_incidents(
    ...
    token_data: TokenData = Depends(require_permissions([PermissionEnum.INCIDENTS_READ])),
...
async def create_incident(
    ...
    incident_service = IncidentService(db)
    incident = await incident_service.create_incident(payload, actor_id=token_data.sub)
...
async def update_status(
    ...
    incident_service = IncidentService(db)
    updated = await incident_service.update_incident_status(incident_id, payload.get("status", "Contained"), actor_id=token_data.sub)
...
async def isolate_asset(
    ...
    incident_service = IncidentService(db)
    asset = await incident_service.isolate_asset(incident_id, asset_id, actor_id=token_data.sub)
