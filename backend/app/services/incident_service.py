...
class IncidentService:
    ...
    async def create_incident(self, payload: IncidentCreateSchema, actor_id: str) -> IncidentResponseSchema:
        ...
        async with self.session.begin():
            ...
            incident = await self.incidents_repo.create(
                ...
                assigned_analyst=payload.assignedAnalyst,
    ...
    async def get_incident_by_id(self, incident_id: str) -> IncidentResponseSchema:
        """Fetch complete incident telemetry matching frontend interface."""
        incident = await self.incidents_repo.get_incident_details(incident_id)
        if not incident:
            raise HTTPException(
    ...
    async def list_incidents(self, status: Optional[str] = None, severity: Optional[str] = None, category: Optional[str] = None) -> List[IncidentResponseSchema]:
        """List active incidents."""
        incidents = await self.incidents_repo.list_incidents(status=status, severity=severity, category=category)
        return [
    ...
    async def update_incident_status(self, incident_id: str, new_status: str, actor_id: str) -> IncidentResponseSchema:
        ...
        async with self.session.begin():
            incident = await self.incidents_repo.get_by_id(incident_id)
            if not incident:
