...
async def test_cases_crud_operations():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        res_create = await client.post("/api/v1/cases", json=payload, headers=headers)
        ...
        # List cases
        res_list = await client.get("/api/v1/cases", headers=headers)
        ...
        # Update case
        res_update = await client.put(f"/api/v1/cases/{case_id}", json={"status": "Closed"}, headers=headers)
        assert res_update.status_code == 200
        assert res_update.json()["data"]["status"] == "Closed"

        # Soft delete case
        res_delete = await client.delete(f"/api/v1/cases/{case_id}", headers=headers)
